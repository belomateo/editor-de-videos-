...
type Cut = { start: number; end: number; razon: string };
...
export default function Editor() {
  ...
  async function load() {
    ...
    setP(data);
    if (data.analysis?.cortes) setEnabledCuts(data.analysis.cortes.map(() => true));
  }
  useEffect(() => { load(); }, [id]);
  ...
  const selectedCuts = p.analysis?.cortes?.filter((_, i) => enabledCuts[i]) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <video
        src={`/api/media?f=${encodeURIComponent(p.sourceFile)}`}
        controls
      ...
      {p.transcript && (
        <section className="card">
          <h2 className="tag mb-2">Transcripción</h2>
          <p className="text-sm text-zinc-300 max-h-40 overflow-auto whitespace-pre-wrap">{p.transcript}</p>
        </section>
      )}
      ...
      {p.analysis && (
        <>
          ...
          <section className="card">
            <h2 className="tag mb-2">Cortes sugeridos ({selectedCuts.length} activos)</h2>
            {p.analysis.cortes.length === 0 && <p className="text-sm text-mute">La IA no encontró nada para cortar. Limpio el crudo.</p>}
            <div className="space-y-1 max-h-56 overflow-auto">
              {p.analysis.cortes.map((c, i) => (
                <label key={i} className="flex items-center gap-3 text-sm py-1 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={enabledCuts[i] ?? true}
                    onChange={() => setEnabledCuts((prev) => prev.map((v, j) => (j === i ? !v : v)))}
                    className="accent-[#FFC857]"
                  />
                  <span className="font-mono text-xs text-mute w-28">{fmt(c.start)} → {fmt(c.end)}</span>
                  <span>{c.razon}</span>
                </label>
              ))}
            </div>
      ...
      {p.clips && p.clips.length > 0 && (
        <section className="card border-amber/40">
          <h2 className="tag mb-2">Clips para Shorts / Reels ({p.clips.length})</h2>
            ...
            {p.clips.map((c, i) => (
              <div key={i} className="bg-ink border border-line rounded-lg p-3">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <button
                    className="btn btn-amber !py-1.5"
                    disabled={!!busy}
                    onClick={() =>
                      call(
                        'render',
                        { id, platform: 'vertical', highlightColor: color, clip: c },
                        `Renderizando "${c.titulo}"…`
                      )
                    }
                  >
                    Renderizar 9:16
        ...
        <section className="card">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-3">
            ...
            <button
              ...
              disabled={!!busy}
              onClick={() =>
                call('render', { id, platform, highlightColor: color, applyCuts: true, selectedCuts }, 'Renderizando (puede tardar unos minutos)…')
              }
            >
              Renderizar
            </button>
                ...
                <div key={r.id} className="bg-ink border border-line rounded-lg p-3">
                  <div className="flex justify-between items-center mb-2 text-xs text-mute flex-wrap gap-2">
                    ...
                    <span className="flex items-center gap-3">
                      {r.driveLink ? (
                        <a className="text-green-400 hover:underline" href={r.driveLink} target="_blank">
                          ✓ En Drive
                        </a>
                      ) : (
                        <button
                          ...
                          disabled={!!busy}
                          onClick={() => call('drive', { id, renderId: r.id }, 'Subiendo a Google Drive…')}
                        >
                          ↑ Subir a Drive
                        </button>
                      )}
                      <a className="text-amber hover:underline" href={`/api/media?f=${encodeURIComponent(r.file)}`} download>
                        Descargar
                      </a>
                    </span>
                  </div>
                  <video
                    src={`/api/media?f=${encodeURIComponent(r.file)}`}
                    controls
